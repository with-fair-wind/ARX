...
struct ThreadSafeLastValue {
    ...
    static result_type combine(std::vector<R>&& results) {  // NOLINT(cppcoreguidelines-rvalue-reference-param-not-moved) : we move an element, not the container
        assert(!results.empty() && "ThreadSafeLastValue combiner requires at least one handler");
...
template <typename Signature, template <typename> class CombinerT = ThreadSafeCollectAll>
class ThreadSafeEvent;

/// ThreadSafeEvent 特化: void 返回值 — 行为与传统 ThreadSafeEvent 完全一致, CombinerT 被忽略.
template <typename... Args, template <typename> class CombinerT>
class ThreadSafeEvent<void(Args...), CombinerT> {
   public:
    ...
    /// 右值引用 Args(如 ThreadSafeEvent<void(T&&)>): 等同 std::move, 单订阅者安全,
    ...
    /// 对于右值引用 Args(如 ThreadSafeEvent<void(T&&)>), flush 时行为与 emit() 一致:
...
/// ThreadSafeEvent 特化: 非 void 返回值 — handler 返回值通过 CombinerT 合并.
...
class ThreadSafeEvent<R(Args...), CombinerT> {
   public:
    using Handler = ThreadSafeDelegate<R(Args...)>;
    using Id = std::size_t;
    ...
    /// 值类型 Args: 每个 handler 收到独立拷贝, 多订阅者安全.
    /// 引用类型 Args: handler 共享引用.
    /// 右值引用 Args(如 ThreadSafeEvent<int(T&&)>): 等同 std::move, 单订阅者安全,
    ///   多订阅者时后续 handler 可能收到被移动后的对象.
...
class ThreadSafeMessageBus {
   public:
    ...
    template <typename Message, typename R, template <typename> class CombinerTC = ThreadSafeCollectAll,
              typename std::enable_if<!std::is_void<R>::value, int>::type = 0>
    ThreadSafeScopedConnection subscribe(std::function<R(const Message&)> func) {
        static_assert(std::is_same<Message, std::decay_t<Message>>::value, "Message type must not be cv-qualified or a reference. Use the base type.");
   ...
   private:
    ...
    struct TypedChannel : IChannel {
        ThreadSafeEvent<void(const Message&)> m_event;
    ...
    struct TypedRetChannel : IChannel {
        ThreadSafeEvent<R(const Message&), CombinerTC> m_event;
        void flush() override { m_event.flush(); }
        std::size_t pending_count() const override { return m_event.pending_count(); }
    };
