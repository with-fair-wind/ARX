...
// eventBus14.hpp — C++14 compatible version of eventBus.hpp
//
// C++17 → C++14 adaptations:
//   std::optional        → std::unique_ptr<T>
//   std::shared_mutex    → std::shared_timed_mutex
//   std::void_t          → detail::void_t
//   std::is_same_v       → std::is_same<>::value
//   std::is_void_v       → std::is_void<>::value
//   std::apply           → detail::apply
//   structured bindings  → .first / .second
...
/// shared_timed_mutex 策略: 支持读写分离, 用于多线程场景.
...
struct HasAccumulator<C, R, void_t<typename C::Accumulator>> : std::true_type {};
...
struct LastValue {
    using result_type = R;
    struct Accumulator {
        std::unique_ptr<R> last_value;
        void reserve(std::size_t) {}
        void add(R&& val) { last_value = std::make_unique<R>(std::move(val)); }
        bool should_stop() const { return false; }
        R finalize() {
            assert(last_value && "LastValue combiner requires at least one handler");
...
struct StopOnTrue {
    ...
    struct Accumulator {
        ...
        void reserve(std::size_t) {}
        void add(R&& val) {
            last_value = std::make_unique<R>(std::move(val));
            if (*last_value) {
                stopped = true;
            }
        }
        ...
        R finalize() {
            assert(last_value && "StopOnTrue combiner requires at least one handler");
            return std::move(*last_value);
        }
    };
};
...
struct StopOnFalse {
    ...
    struct Accumulator {
        ...
        bool should_stop() const { return stopped; }
        R finalize() {
            assert(last_value && "StopOnFalse combiner requires at least one handler");
            return std::move(*last_value);
        }
    };
};
...
class Event<void(Args...), CombinerT, LockPolicy> : private detail::MovePolicy<true> {
   ...
   public:
    ...
    void flush() {
        ...
        detail::for_each_safe(batch.begin(), batch.end(), [this](auto& stored) { detail::apply([this](auto&&... args) { emit(static_cast<Args>(std::move(args))...); }, std::move(stored)); });
    }
...
class Event<R(Args...), CombinerT, LockPolicy> : private detail::MovePolicy<true> {
   ...
   public:
    ...
    std::vector<ResultType> flush() {
        ...
        detail::for_each_safe(batch.begin(), batch.end(), [&](auto& stored) { all_results.push_back(detail::apply([this](auto&&... args) { return emit(static_cast<Args>(std::move(args))...); }, std::move(stored))); });
        return all_results;
    }
...
class BasicMessageBus : private detail::MovePolicy<std::is_same<LockPolicy, NoLock>::value> {
   public:
    ...
    ScopedConnection subscribe(std::function<void(const Message&)> func) {
        static_assert(std::is_same<Message, typename std::decay<Message>::type>::value, "Message type must not be cv-qualified or a reference.");
    ...
    template <typename Message, typename R, template <typename> class CombinerTC = CollectAll, typename std::enable_if<!std::is_void<R>::value, int>::type = 0>
    ...
    void post(Message&& message) {
        using Decayed = typename std::decay<Message>::type;
