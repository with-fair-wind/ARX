...
void SampleDlg::Display(AcDbCircle* pObj)
{
	...
	AddToList(_T("Center Pt"), rad);
	AddToList(_T("Radius"), centerpt);
...
{
	// Hide the dialog box and give control to the CAD
	BeginEditorCommand();
	// User interaction
	ads_name en;
	ads_point pt;
	...
	{
		// Cancel command(including dialog box) if interaction is invalid
		CancelEditorCommand();
	}
	else
	{
		// If the interaction is valid, continue to execute
