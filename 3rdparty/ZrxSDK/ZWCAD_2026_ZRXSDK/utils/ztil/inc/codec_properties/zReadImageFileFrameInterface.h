...
namespace Ztil { class RowIteratorInterface; }
#endif

#ifndef ZREADIMAGEFILEFRAMEINTERFACE_H
#define ZREADIMAGEFILEFRAMEINTERFACE_H
#if  __GNUC__ >= 4
...
class DllImpExp ReadImageFileFrameInterface 
{
public:
    ///<summary>This enum describes the different methods ZTIL accepts data in.</summary>
    enum AccessMethod { 
        ...
        ///ZTIL will call this for sub sampled data if scaled (lower) data is requested.</remarks>
        kSubSampled 
    };
public:
    ...
    ///<param name='nRows'>A reference to an integer where the number image rows will be stored.</param>
    ///<param name='nColumn'>A reference to an integer where the number image columns will be stored.</param>
    ///<param name='nDepths'>A reference to an integer where the number image depths will be stored.</param>
    ...
    ///<remarks>ZTIL supports a discreet number of pixel sizes.</remarks>
    ...
    ///<returns>This returns a member of the <c>Ztil::Orientation</c> enum describing the direction of rows and columns for the data.</returns>
    ...
    virtual Ztil::Orientation dataOrientation () = 0;
    ...
    /// <summary>This will query for a specific property from the current frame of the image.</summary>
    /// <param name="sbPropertyName">An instance of a <c>Ztil::StringBuffer</c> that holds the name of the property type to return.</param>
    /// <returns>An instance of a <c>FormatCodecPropertyInterface</c> derived property that
    /// should be deleted by the caller. A NULL pointer will be returned if the requested property can not be found.</returns>
