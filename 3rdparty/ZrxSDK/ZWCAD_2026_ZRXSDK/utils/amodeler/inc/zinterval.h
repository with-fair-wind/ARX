...
public:
    ...
    {
        if (a.x < b.x) { lower.x = a.x; upper.x = b.x; }
        else { lower.x = b.x; upper.x = a.x; }
        if (a.y < b.y) { lower.y = a.y; upper.y = b.y; }
        else { lower.y = b.y; upper.y = a.y; }
    }
...
public:
    ...
    ZwInterval3d(ZwPoint3d a, ZwPoint3d b)
    {
        ...
        if (a.z < b.z) { lower.z = a.z; upper.z = b.z; }
        else { lower.z = b.z; upper.z = a.z; }
    }
...
public:
    ...
    bool			isNull() const		{ return lower.x > upper.x || lower.y > upper.y; }
    ZwIntPoint2d	mid() const			{ return ZwIntPoint2d((upper.x + lower.x) / 2, (upper.y + lower.y) / 2); }
    void			init()				{ *this = kNull; }
...
public:
    ...
    int				length(int i) const	{ return upper[i] - lower[i]; }
    bool			isNull() const		{ return lower.x > upper.x || lower.y > upper.y || lower.z > upper.z; }
    ZwIntPoint3d	mid() const			{ return ZwIntPoint3d((upper.x + lower.x) / 2, (upper.y + lower.y) / 2, (upper.z + lower.z) / 2); }
