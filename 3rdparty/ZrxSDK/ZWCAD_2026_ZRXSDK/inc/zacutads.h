...
int            zcutRbDup(const struct resbuf *source, struct resbuf **result);
...
int zcutVSPrintf(wchar_t *buf, size_t nBufLen, const wchar_t *fmt, va_list va);

template<size_t nBufLen> inline int zcutSPrintf(wchar_t (& buf)[nBufLen], const ZTCHAR *format, ...)
{
    va_list va;
    va_start(va, format);
...
template<size_t nBufLen> inline int zcutVSPrintf(wchar_t (& buf)[nBufLen], const ZTCHAR *format,
                                                 va_list va)
{
