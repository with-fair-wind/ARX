...
class ZCDBCORE2D_PORT Services
{
public:
...
inline Zcad::ErrorStatus getSymbolName(ZcString & sName, ZcDbObjectId objId)
{
    sName.setEmpty();
...
inline Zcad::ErrorStatus getSymbolName(ZTCHAR *&pName, ZcDbObjectId objId)
{
    ZcString sName;
    return ::zcutZcStringToZTChar(sName, pName, ZcDbSymbolUtilities::getSymbolName(sName, objId));
}
...
ZCDBCORE2D_PORT const Services * ZCDBSYMUTIL_SERVICES_NAME();
...
ZCDBCORE2D_PORT Zcad::ErrorStatus  getTableStyleNameFromDbById(const ZcDbObjectId& objId, ZcString& sName);

inline Zcad::ErrorStatus  getTableStyleNameFromDbById(const ZcDbObjectId& objId, ZTCHAR*& pName)
{
    ZcString sName;
    return ::zcutZcStringToZTChar(sName, pName, ::getTableStyleNameFromDbById(objId, sName));
}
