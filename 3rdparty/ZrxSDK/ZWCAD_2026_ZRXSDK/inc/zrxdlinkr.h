...
class ZCBASE_PORT ZSOFT_NO_VTABLE ZcRxDynamicLinker: public ZcRxObject
{
public:
    ...
    virtual ZcLocale       ProductLocale() const =0;

    virtual ZcadApp::ErrorStatus registerApp(ZcadApp::LoadReasons alr, const ZTCHAR  *logicalName, void* entryPoint = nullptr) = 0;
    virtual ZcadApp::ErrorStatus unregisterApp(const ZTCHAR  *logicalName) = 0;
};

#define zcrxDynamicLinker                                               \
ZcRxDynamicLinker::cast(zcrxSysRegistry()->at(ZCRX_DYNAMIC_LINKER))

class ZCBASE_PORT ZSOFT_NO_VTABLE ZcRxDLinkerReactor: public ZcRxObject
