...
class ZCBASE_PORT ZdCharFormatter
{
  public:
    enum {
        kUnknown = 0,
   ...
   static unsigned lookupBOM(unsigned nBom)
	...
	static unsigned wcharToUtf16(wchar_t wch,
		char * pDestBuf,
		unsigned nDestBufSize,
		bool bLittleEndian,
		bool bExpandLF);
    ...
    template <class ChType> ZSOFT_DEPRECATED static bool putMIF(wchar_t wch,
	...
	{
		return
			ch == '1' ? code_page_id::CODE_PAGE_DOS932 :  
                                                 ...
                                                 wchar_t &wch)
    {
        ...
        const int nCvted = MultiByteToWideChar(
            getMIFCodePage<ChType>(pSrcBuf[3]),
	...
	static int isNativeToCodePageId(wchar_t wch,  
		code_page_id codePageId,     
		char *pChNative,   
		int bufferLen);		


    static int wcharNonNativeToCIF(const wchar_t *szSrc,
        wchar_t *szDst,
        size_t cchDstSize,
