...
class ZCDBCORE2D_PORT ZcDbTable: public ZcDbBlockReference
{
public:
    ...
    virtual void              objectClosed(const ZcDbObjectId objId) override;
    virtual void              erased(const ZcDbObject* dbObj,
                                      ZSoft::Boolean pErasing = true) override;

    virtual Zcad::ErrorStatus setPosition(const ZcGePoint3d&) override;

    virtual Zcad::ErrorStatus setNormal(const ZcGeVector3d& newVal) override;
	...
	double          rotation                (int row, 
											int col, 
											int nContent) const;
	Zcad::ErrorStatus setRotation           (double fAngle) override;
