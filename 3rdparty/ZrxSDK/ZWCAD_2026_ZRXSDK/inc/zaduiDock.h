...
#define WM_ZCAD_DOCKBAR_ROLLED_UP  (WM_USER+37)
#define WM_ZCAD_DOCKBAR_ROLLED_OUT (WM_USER+38)
...
class ZDUI_PORT CZdUiDockControlBarThemeModReactor : public CZdUiThemeModifiedReactor  
{
public:
    CZdUiDockControlBarThemeModReactor(CZdUiDockControlBar* pDockControlBar);
	virtual void ThemeModified(const CZdUiTheme * pTheme, ZdUiThemeElement element) override;
    void SetDockControlBar(CZdUiDockControlBar* pDockControlBar);

private:
    CZdUiDockControlBar * m_pDockControlBar;
};
...
class ZDUI_PORT CZdUiDockControlBar : public CControlBar
{
    ...
    friend class CZdUiDockControlBarThemeModReactor;
...
private:
    ...
    CZdUiBitmapButton m_closeBtn;
    CZdUiBitmapButton m_pinBtn;
    CLSID             m_ID;
...
public:
    ...
    static bool HidePalettes();
    static bool ShowPalettes();
        void GetAllPaletteSetsRgn(CRgn& rgnPSWnd);
public:
    enum ContainedBarType { Unknown, Standard, ZwSoftBar } ;
...
private:
    ...
    void    doDrawButton(CDC *pDC, ZdUiSkBtnItems btnItem, BOOL bErase = false);
    ...
    void        SetRollupTimer();
    bool        shouldWindowBeRolledUp( CWnd* pWndUnderCursor, CZdUiPaletteSet& ps) const;
...
bool ZDUI_PORT ZdUiRegisterTool (
    LPCTSTR lpszToolName, LPCTSTR lpszCommand, LPCGUID pID
);
...
class ZDUI_PORT CZdUiPaletteSetDockSite {
public:
