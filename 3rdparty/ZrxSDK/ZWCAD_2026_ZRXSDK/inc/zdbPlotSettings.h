...
class ZCDBCORE2D_PORT ZcDbPlotSettings: public ZcDbObject
{
public:
    ...
    Zcad::ErrorStatus
    getPlotPaperSize           (ZcGePoint2d& paperSize) const;
    Zcad::ErrorStatus 
    getCanonicalMediaName      (const ZTCHAR *& mediaName) const;

    Zcad::ErrorStatus getCanonicalMediaName(ZcString& mediaName) const;

    Zcad::ErrorStatus 
    getPlotOrigin              (double& x,
                                double& y) const;
    Zcad::ErrorStatus
    getPlotOrigin              (ZcGePoint2d& origin) const;
	...
	void              
	setPlotTransparency        (ZSoft::Boolean plotTransparency);
    ...
    Zcad::ErrorStatus
    setShadePlot               (ZcDbPlotSettings::ShadePlotType type,
                                ZcDbObjectId shadePlotId);
