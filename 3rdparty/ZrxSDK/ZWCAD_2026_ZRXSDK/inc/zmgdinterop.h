...
class ZCBASE_PORT ZcMgObjectFactoryBase : public ZcRxObject
{
public:
...
public:
    ...
    ZcMgObjectFactory()
    {
		auto desc = ImpObj::desc();
		if (desc)
		{
			desc->addX(ZcMgObjectFactoryBase::desc(), this);
		}
    }
    ~ZcMgObjectFactory()
    {
        auto desc = zcrxFindZcRxClass(ImpObj::className());
		if (desc)
		{
			desc->delX(ZcMgObjectFactoryBase::desc());
		}
    }
	virtual gcroot<Type_GcPtr> getType() override
    {
#ifdef __cplusplus_cli
        return RCW::typeid;
#else
