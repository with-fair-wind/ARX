...
public:
    ...
    {
		auto locale = zcrxProductLocale();

		ZcLocale::LocaleId lcid = locale;
    ...
    {
		ZSoft::IntDbId val;
		auto hr = ZcAxGetOwnerId(m_objRef, &val);
		*pVal = val;
		return hr;
    }
...
template <class C, const CLSID* pclsid, class T, const IID* piid, const GUID* plibid,
	WORD wMajor = 1, WORD wMinor = 0, class tihclass = CComTypeInfoHolder>
class ATL_NO_VTABLE IZcadSurfaceDispatchImpl :
	public IZcadEntityDispatchImpl<C, pclsid, T, piid, plibid, wMajor, wMinor, tihclass>
{
public:
	STDMETHOD(get_UIsolineDensity)(long *density)
	{
		return ZcAxGet_UIsolineDensity(m_objId, density);
	}

	STDMETHOD(put_UIsolineDensity)(long density)
	{
		return ZcAxPut_UIsolineDensity(m_objId, density);
	}

	STDMETHOD(get_VIsolineDensity)(long *density)
	{
		return ZcAxGet_VIsolineDensity(m_objId, density);
	}

	STDMETHOD(put_VIsolineDensity)(long density)
	{
		return ZcAxPut_VIsolineDensity(m_objId, density);
	}

	STDMETHOD(get_WireframeType)(ZcWireframeType *type)
	{
		return ZcAxGet_WireframeType(m_objId, type);
	}

	STDMETHOD(put_WireframeType)(ZcWireframeType type)
	{
		return ZcAxPut_WireframeType(m_objId, type);
	}

	STDMETHOD(get_MaintainAssociativity)(int *maintainAssoc)
	{
		return ZcAxGetMaintainAssociativity(m_objId, maintainAssoc);
	}

	STDMETHOD(put_MaintainAssociativity)(int maintainAssoc)
	{
		return ZcAxPutMaintainAssociativity(m_objId, maintainAssoc);
	}

	STDMETHOD(get_ShowAssociativity)(VARIANT_BOOL *bEnabled)
	{
		return ZcAxGetShowAssociativity(m_objId, bEnabled);
	}

	STDMETHOD(put_ShowAssociativity)(VARIANT_BOOL bEnabled)
	{
		return ZcAxPutShowAssociativity(m_objId, bEnabled);
	}

	STDMETHOD(get_EdgeExtensionDistances)(VARIANT *extDistances)
	{
		return ZcAxGetEdgeExtensionDistances(m_objId, extDistances);
	}

	STDMETHOD(put_EdgeExtensionDistances)(VARIANT extDistances)
	{
		return ZcAxPutEdgeExtensionDistances(m_objId, extDistances);
	}

	STDMETHOD(get_SurfTrimAssociativity)(VARIANT *associative)
	{
		return ZcAxGetSurfTrimAssociativity(m_objId, associative);
	}

	STDMETHOD(put_SurfTrimAssociativity)(VARIANT associative)
	{
		return ZcAxPutSurfTrimAssociativity(m_objId, associative);
	}
};
...
public:
	...
	virtual HRESULT CreateNewObject(ZcDbObjectId& objId,
		ZcDbObjectId& ownerId, ZTCHAR* keyName)
	{
		return E_NOTIMPL;
	}
