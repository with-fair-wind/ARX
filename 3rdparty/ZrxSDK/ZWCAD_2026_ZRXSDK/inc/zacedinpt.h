...
class ZcEdInputPoint;
class ZcEdInputPointFilterResult;
class ZcEdInputPointMonitorResult;
...
{
private:
	ZcEdImpInputPointManager* m_pImpMgr = nullptr;
...
{
private:
	ZcEdImpInputPointFilterResult* m_pImp = nullptr;
...
{
private:
	ZcEdImpInputPointMonitorResult* m_pImp = nullptr; 
...
class ZCAD_PORT  ZcEdInputPointFilter : public ZcRxObject
{

  public:
    ZCRX_DECLARE_MEMBERS(ZcEdInputPointFilter);
    virtual ~ZcEdInputPointFilter() {}

    ZSOFT_DEPRECATED virtual Zcad::ErrorStatus processInputPoint(
          bool&                        changedPoint,
		  ...
		  virtual Zcad::ErrorStatus processInputPoint(const ZcEdInputPoint& input, ZcEdInputPointFilterResult& output);
		  virtual ZcEdInputPointFilter* revokeInputFilter(ZcEdInputPointFilter* revokedFilter)                { return this; }
          virtual bool mouseHasMoved();
private:
    ZcEdDSPoint m_lastPoint{ 0,0 };
};

class ZCAD_PORT  ZcEdInputPointMonitor : public ZcRxObject
{
    public:
    ZCRX_DECLARE_MEMBERS(ZcEdInputPointMonitor);
    virtual ~ZcEdInputPointMonitor() {}

    ZSOFT_DEPRECATED virtual Zcad::ErrorStatus monitorInputPoint(
          ...
          const ZcGePoint3d&                  computedPoint,
          const ZTCHAR*                         tooltipString) {	return Zcad::eOk; }
		  virtual Zcad::ErrorStatus monitorInputPoint(const ZcEdInputPoint& input, ZcEdInputPointMonitorResult& output);
...
{
public:
	ZCAD_PORT virtual ~ZcEdInputContextReactor() { }
    ...
    virtual void undoLastPoint(const ZcGePoint3d*) { }
};
